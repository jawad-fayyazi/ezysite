<section>

    <div class="relative w-full text-center">
        <h2 class="text-3xl sm:text-4xl text-left md:text-center font-medium tracking-tighter lg:text-5xl">
            <span class='gradient-txt'>Launch Your Ideas</span> <br> <span class='text-black'>Faster than Ever</span>
        </h2>
        <p class="mt-4 text-sm sm:text-base font-medium text-left md:text-center md:text-balance text-zinc-500 mx-auto max-w-2xl">
            Empower your business with a fast and seamless website creation tool. Build and launch your site in no time, with zero hassle.
        </p>
    </div>



<div class="text-center">
    <div class="grid grid-cols-2 gap-x-6 gap-y-12 mt-12 text-center lg:mt-16 lg:grid-cols-4 lg:gap-x-8 lg:gap-y-16">
        <!-- User Management -->
        <div>
            <div class="flex justify-center items-center mx-auto gradient-circle rounded-full w-12 h-12">
                <x-phosphor-users-three class="w-6 h-6 text-white" />
            </div>
            <div class="mt-6">
                <h3 class="font-medium text-zinc-900">User Management</h3>
                <p class="mt-2 text-sm text-zinc-500">
                    Effortlessly manage users with Wave's intuitive dashboard.
                </p>
            </div>
        </div>

        <!-- Secure Authentication -->
        <div>
            <div class="flex justify-center items-center mx-auto gradient-circle rounded-full w-12 h-12">
                <x-phosphor-shield-check class="w-6 h-6 text-white" />
            </div>
            <div class="mt-6">
                <h3 class="font-medium text-zinc-900">
                    Secure Authentication
                </h3>
                <p class="mt-2 text-sm text-zinc-500">
                    Implement secure login, registration, and two-factor authentication.
                </p>
            </div>
        </div>

        <!-- Third-Party Integrations -->
        <div>
            <div class="flex justify-center items-center mx-auto gradient-circle rounded-full w-12 h-12">
                <x-phosphor-puzzle-piece class="w-6 h-6 text-white" />
            </div>
            <div class="mt-6">
                <h3 class="font-medium text-zinc-900">Third-Party Integrations</h3>
                <p class="mt-2 text-sm text-zinc-500">
                    Easily integrate with popular third-party services.
                </p>
            </div>
        </div>

        <!-- Analytics -->
        <div>
            <div class="flex justify-center items-center mx-auto gradient-circle rounded-full w-12 h-12">
                <x-phosphor-chart-bar class="w-6 h-6 text-white" />
            </div>
            <div class="mt-6">
                <h3 class="font-medium text-zinc-900">Analytics</h3>
                <p class="mt-2 text-sm text-zinc-500">
                    Gain insights into user behavior with built-in analytics.
                </p>
            </div>
        </div>

        <!-- Customizable Themes -->
        <div>
            <div class="flex justify-center items-center mx-auto gradient-circle rounded-full w-12 h-12">
                <x-phosphor-paint-bucket class="w-6 h-6 text-white" />
            </div>
            <div class="mt-6">
                <h3 class="font-medium text-zinc-900">Customizable Themes</h3>
                <p class="mt-2 text-sm text-zinc-500">
                    Personalize your app with easy-to-use themes.
                </p>
            </div>
        </div>

        <!-- Easy Setup -->
        <div>
            <div class="flex justify-center items-center mx-auto gradient-circle rounded-full w-12 h-12">
                <x-phosphor-gear-fine class="w-6 h-6 text-white" />
            </div>
            <div class="mt-6">
                <h3 class="font-medium text-zinc-900">Easy Setup</h3>
                <p class="mt-2 text-sm text-zinc-500">
                    Get started quickly with Wave's straightforward setup process.
                </p>
            </div>
        </div>

        <!-- Community Support -->
        <div>
            <div class="flex justify-center items-center mx-auto gradient-circle rounded-full w-12 h-12">
                <x-phosphor-lifebuoy class="w-6 h-6 text-white" />
            </div>
            <div class="mt-6">
                <h3 class="font-medium text-zinc-900">Community Support</h3>
                <p class="mt-2 text-sm text-zinc-500">
                    Join a vibrant community of developers for support and collaboration.
                </p>
            </div>
        </div>

        <!-- File Management -->
        <div>
            <div class="flex justify-center items-center mx-auto gradient-circle rounded-full w-12 h-12">
                <x-phosphor-images-square class="w-6 h-6 text-white" />
            </div>
            <div class="mt-6">
                <h3 class="font-medium text-zinc-900">File Management</h3>
                <p class="mt-2 text-sm text-zinc-500">
                    Manage your files efficiently with Wave's built-in file manager.
                </p>
            </div>
        </div>
    </div>
</div>

</section>