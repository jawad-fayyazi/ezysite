...
new class extends Component implements HasForms {
    ...
    public function mount($project_id): void
    {
        ...
        if ($this->project->live) {
            ...
            $this->liveData["pages"] = $this->pages
                ->where("live", true)
                ->pluck("id")
                ->toArray();
        ...
        foreach ($this->pages as $page) {
            ...
            if (empty($page["name"])) {
                Notification::make()
                    ...
                    ->title("Error")
                    ->body(
                        "Template's Page 'name' field is missing or invalid."
                    )
                    ->send();
                return;
            }
            ...
            if (is_null($page["page_id"])) {
                Notification::make()
                    ...
                    ->title("Error")
                    ->body(
                        "Template's Page 'page_id' field is missing or invalid."
                    )
                    ->send();
                return;
            }
            ...
            if (empty($page["slug"]) || !is_string($page["slug"])) {
                Notification::make()
                    ...
                    ->title("Error")
                    ->body(
                        "Template's Page 'slug' field is missing or invalid."
                    )
                    ->send();
                return;
            }
            ...
            if (empty($page["title"]) || !is_string($page["title"])) {
                Notification::make()
                    ...
                    ->title("Error")
                    ->body(
                        "Template's Page 'title' field is missing or invalid."
                    )
                    ->send();
                return;
            }
            ...
            if (empty($page["html"]) || !is_string($page["html"])) {
                Notification::make()
                    ...
                    ->title("Error")
                    ->body(
                        "Template's Page 'html' field is missing or invalid."
                    )
                    ->send();
                return;
            }
    ...
    public function saveAsPublicTemplate(): void
    {
        ...
        $template = Template::create([
            "template_name" =>
                $this->project->project_name . " (Public Template)",
            "description" => $this->project->description,
            "template_json" => $projectJson, // Use the project JSON for the new template
            "header_embed" => $headerEmbedGlobal,
            "footer_embed" => $footerEmbedGlobal,
    ...
    public function duplicate(): void
    {
        ...
        while (Project::where("project_name", $newProjectName)->exists()) {
            ...
            $newProjectName =
                $this->project->project_name . " (" . $counter . ")";
            $counter++;
        }
    ...
    public function pageUpdate($pageId)
    {
        ...
        if ($page) {
            ...
            if (isset($data["page_title"]) && !empty($data["page_title"])) {
                ...
                $title =
                    $data["page_name"] . " - " . $this->project->project_name;
            }
    ...
    public function pageMain($pageId)
    {
        ...
        if ($pageInstance) {
            ...
            WebPage::where("website_id", $pageInstance->website_id)->update([
                "main" => false,
            ]);
    ...
    document.body.appendChild(ezysiteLabel);
        ...
        if (!$this->mainPage) {
            if ($this->shouldRedirect) {
                Notification::make()
                    ...
                    ->title("No Main Page")
                    ->body(
                        "You must select a main page for your website to preview."
                    )
                    ->send();
            ...
            if (!mkdir($targetFolder, 0755, true)) {
                if ($this->shouldRedirect) {
                    Notification::make()
                        ...
                        ->body(
                            "Failed to create preview for the domain: {$domain}"
                        )
                        ->send();
        ...
        if ($this->shouldRedirect) {
            Notification::make()
                ->success()
                ->title(
                    "Website successfully made live at " .
                        $domain .
                        $this->ourDomain
                )
                ->send();
...
<script>
    ...
    document.body.appendChild(ezysiteLabel);
        ...
        if (!$this->check()) {
            if ($this->shouldRedirect) {
                Notification::make()
                    ...
                    ->title("Domain Error")
                    ->body(
                        "You must select correct domain for your website to live."
                    )
                    ->send();
        ...
        if (empty($pages)) {
            if ($this->shouldRedirect) {
                Notification::make()
                    ...
                    ->body(
                        "You must select at least one page to make the website live."
                    )
                    ->send();
        ...
        if (!$mainPageExists) {
            if ($this->shouldRedirect) {
                Notification::make()
                    ...
                    ->title("No Main Page")
                    ->body(
                        "You must select a main page for your website to live."
                    )
                    ->send();
            ...
            if (!mkdir($targetFolder, 0755, true)) {
                if ($this->shouldRedirect) {
                    Notification::make()
                        ...
                        ->title("Domain Configuration Error")
                        ->body(
                            "Failed to create website for the domain: {$domain}"
                        )
                        ->send();
        ...
        if (!file_put_contents($robotsFilePath, $robotsTxt)) {
            if ($this->shouldRedirect) {
                Notification::make()
                    ...
                    ->body(
                        "Failed to create robots.txt for the domain: {$domain}"
                    )
                    ->send();
        ...
        // Process the HTML to replace base64 images
        $footerHtml = preg_replace_callback(
            '/<img[^>]+src=["\']([^"\']+)["\']/i',
            function ($matches) use ($domain) {
                $imageSrc = $matches[1];
        ...
        if (!file_put_contents($sitemapFilePath, $sitemap)) {
            if ($this->shouldRedirect) {
                Notification::make()
                    ...
                    ->title("Sitemap Error")
                    ->body(
                        "Failed to create sitemap.xml for the domain: {$domain}"
                    )
                    ->send();
        ...
        foreach ($pages as $pageId) {
            ...
            // Process the HTML to replace base64 images
            $updatedPageHtml = preg_replace_callback(
                '/<img[^>]+src=["\']([^"\']+)["\']/i',
                function ($matches) use ($domain) {
                    $imageSrc = $matches[1];
        ...
        $callback = function ($matches) use ($pageSlug) {
            ...
            if ($linkWithoutExt === $pageSlugWithoutExt) {
                ...
                $newClass = $existingClass
                    ? $existingClass . " active-menu-item"
                    : "active-menu-item";
                $classAttribute = 'class="' . $newClass . '"';

                // Add or replace the class attribute
                $beforeAttributes = preg_replace(
                    '/class\s*=\s*"[^"]*"/i',
                    "",
                    $matches[1]
                );
                $afterAttributes = preg_replace(
                    '/class\s*=\s*"[^"]*"/i',
                    "",
                    $matches[3]
                );
                return "<a " .
                    trim($beforeAttributes) .
                    " " .
                    $classAttribute .
                    " " .
                    trim($afterAttributes) .
                    ' href="' .
                    $matches[2] .
                    '">' .
                    $matches[4] .
    ...
    private function buildFullHtml($data)
    {
        try {
            ...
            $data["header_html"] = $this->addActiveClassToMenu(
                $data["header_html"],
                $data["page_slug"]
            );
        } catch (Exception $e) {
            ...
            Notification::make()
                ...
                ->body(
                    "Failed to update menu items with the active class. Please try again later."
                )
                ->send();
        }
        ...
        if (file_exists($targetFolder)) {
            ...
            if ($this->shouldRedirect) {
                ...
                Notification::make()
                    ...
                    ->body(
                        "There is no active website associated with this domain."
                    )
                    ->send();
                // Redirect or refresh logic
                return redirect("/websites" . "/" . $this->project->project_id);
  ...
  <path d="M6 8l2 2 4-4" stroke="green" fill="none"/>
    ...
    public function unsettAndDeleteOgImg($pageId)
    {
        if (isset($this->pageData[$pageId]["og_img"])) {
            $dir =
                "/var/www/ezysite/public/storage/usersites/" .
                $this->project->project_id .
                "/og_img/" .
   ...
   <div x-show="activeTab === 'pages' && editingPage" class="card p-6">
      <div class="flex justify-between items-center mb-6">
         ...
         <button 
            ...
            >
         <span class="h-5 w-5 font-bold text-gray-600 dark:text-gray-400">✕</span>
         </button>
      </div>
      @foreach($this->pages as $page)
              ...
              <div class="flex justify-end gap-x-3">
                ...
                <x-dropdown class="text-gray-500">
                  ...
                  <a href="#" wire:click="duplicate"
                    ...
                    <x-icon name="phosphor-copy" class="w-4 h-4 mr-2" />
                    Duplicate Website
                  </a>
                  <!-- Save as My Template -->
                ...
                <div x-show="open" x-transition.duration.300ms x-cloak class="mt-4 space-y-3 bg-white-100 p-4 rounded-lg rounded-md shadow-md">
                  ...
                  <div class="form-group grid gap-y-2">
                    <div x-data="{
                      ...
                      <!-- Image Preview Section -->
                      <div class="relative mt-3 z-30">
                        <img src="{{ asset('storage/usersites/'.$this->project->project_id.'/og_img/'.$pageData[$page->id]['og_img'])}}" alt="Image Preview" class="max-w-xs h-auto rounded shadow-lg mx-auto" style="max-height: 12.5rem;" />
                      </div>
                      @else
                      <!-- Hidden File Input -->
                      <input 
